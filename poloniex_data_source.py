...
class PoloniexDataSource(object):
    ...
    def __init__(self, asset_data, client = Poloniex, api_key = "", secret_key = ""):
        ...
        self.chart_data = pd.DataFrame.from_dict(self._chart_data())
    ...
    def _chart_data(self):
        return self.client.returnChartData(
                self.pair,
                self.asset_data["period"],
                self.asset_data["start_date"],
                self.asset_data["end_date"]
                )
